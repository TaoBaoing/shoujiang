...
namespace Com.Alipay
{
    ...
    public class AlipayCore
    {
        ...
        /// </summary>
        /// <param name="parameters">需要拼接的数组</param>
        /// <returns>拼接完成以后的字符串</returns>
        public static string CreateLinkUrlString(SortedDictionary<string, string> parameters,bool filterSign)
        {
            ...
            foreach (KeyValuePair<string, string> item in parameters)
            {
                ...
                if (!string.IsNullOrWhiteSpace(item.Value))
                {
                    ...
                    prestr.AppendFormat("{0}={1}&", item.Key, value);
                }
            }
            //去掉最后一个&字符
        ...
        public static string CreateLinkStringWithOutYinHao(Dictionary<string, string> dicArray)
        {
            StringBuilder prestr = new StringBuilder();
            foreach (KeyValuePair<string, string> temp in dicArray)
            {
